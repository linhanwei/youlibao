<?php 
 ...
 //导入excel内容转换成数组 
  ...
  $PHPExcel = $PHPReader->load($filePath,'utf-8'); 
  ...
  /**从第二行开始输出，因为excel表中第一行为列名*/ 
   $i = 0;
  for($currentRow = self::$config['currentRow'];$currentRow <= $allRow;$currentRow++){      
  	/**从第A列开始输出*/ 
  	for($currentColumn= 0;$currentColumn<= count($fields); $currentColumn++){ 	
