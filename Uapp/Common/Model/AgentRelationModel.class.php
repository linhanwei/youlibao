...
use Think\Model;
class AgentRelationModel extends Model {
	//定义数据表字段
//	protected $fields = array();
        
        //数据表
        protected $trueTableName = 'agent_relation'; 
...
//	);
        ...
        public function getList($where=array(),$limit=10,$page=1,$order='',$field=array('field'=>array(),'is_opposite'=>false),$cache=array('key'=>false,'expire'=>null,'cache_type'=>null),$join=''){
            ...
            if($list){
                ...
                foreach ($list as $k => $v) {
                    $list[$k]['agent_lv_name'] = $MEMBER_LEVEL[$v['agent_grade']]['name'];
                    switch ($v['is_validate']) {
                        case 1:
                            $is_validate_name = '已审核';
                            break;
                        case 0:
                            $is_validate_name = '待总部审核';
                            break;
                        case -1:
                            $is_validate_name = '黑名单';
                            break;
                        case -2:
                            $is_validate_name = '待上级审核';
                            break;

                        default:
                            $is_validate_name = '驳回修改';
                            break;
                    }
                    $list[$k]['is_validate_name'] = $is_validate_name;
                }
            }
         ...
         * $cache 缓存条件
         */
        public function getCount($where=array(),$cache=array('key'=>false,'expire'=>null,'cache_type'=>null),$join='') {
