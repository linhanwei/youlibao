...
CREATE TABLE `agent` (
  ...
  `openid` varchar(150) DEFAULT NULL COMMENT '公众号openid',
  PRIMARY KEY (`agentId`),
  KEY `name` (`name`),
  KEY `weixin` (`weixin`),
  KEY `star` (`star`),
  KEY `tel` (`tel`),
  KEY `openid` (`openid`)
) ENGINE=InnoDB AUTO_INCREMENT=922 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC;
...
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='代理等级与商品分润关系表';
...
CREATE TABLE `agent_goods_stock_rale` (
  ...
  KEY `goods_stock` (`goods_stock`)
) ENGINE=InnoDB AUTO_INCREMENT=467 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='代理与产品库存关系表';
...
CREATE TABLE `agent_month_profit` (
  ...
  `is_cash` tinyint(1) DEFAULT '2' COMMENT '是否提现: 1:是,2:否',
  ...
  KEY `agent_id` (`agent_id`),
  KEY `is_cash` (`is_cash`)
) ENGINE=InnoDB AUTO_INCREMENT=476 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='代理每月销售与进货与分润总额报表';
...
CREATE TABLE `agent_profit_log` (
  ...
  KEY `year` (`year`),
  KEY `month` (`month`)
) ENGINE=InnoDB AUTO_INCREMENT=6784 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='代理分润记录表';
...
) ENGINE=InnoDB AUTO_INCREMENT=803 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC;
...
CREATE TABLE `cards` (
  ...
  KEY `member_id` (`member_id`)
) ENGINE=InnoDB AUTO_INCREMENT=2672 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='购物车';
...
CREATE TABLE `cash_prize_log` (
  ...
  `out_time` datetime DEFAULT NULL COMMENT '过期时间\r\n',
  `add_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '添加时间\r\n',
  PRIMARY KEY (`id`),
  KEY `agent_id` (`agent_id`),
  KEY `prize_code` (`prize_code`)
) ENGINE=InnoDB AUTO_INCREMENT=85 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='特约扫码兑换奖品';
...
CREATE TABLE `company_payment_agent_log` (
  ...
  `weixin` varchar(50) DEFAULT NULL COMMENT '代理微信号',
  `bank_account` varchar(50) DEFAULT NULL COMMENT '代理银行账户\r\n',
  `bank_name` varchar(100) DEFAULT NULL COMMENT '代理银行账户名称\r\n',
  `money` int(10) unsigned DEFAULT '0' COMMENT '转账金额\r\n',
  `profit_year` mediumint(4) DEFAULT NULL COMMENT '分润的年',
  `profit_month` tinyint(2) DEFAULT NULL COMMENT '分润的月',
  `year` mediumint(4) unsigned DEFAULT NULL COMMENT '年',
  `month` tinyint(2) DEFAULT NULL COMMENT '月',
  `day` tinyint(2) DEFAULT NULL COMMENT '日',
  `openid` varchar(150) DEFAULT NULL COMMENT '商户appid下，某用户的openid',
  `partner_trade_no` varchar(32) DEFAULT NULL COMMENT '商户订单号，需保持唯一性',
  `payment_no` varchar(50) DEFAULT NULL COMMENT '企业付款成功，返回的微信订单号',
  `payment_time` datetime DEFAULT NULL COMMENT '企业付款成功时间',
  `status` tinyint(1) DEFAULT '1' COMMENT '付款是否成功:1:否,2:是',
  ...
  KEY `partner_trade_no` (`partner_trade_no`),
  KEY `payment_no` (`payment_no`),
  KEY `weixin` (`weixin`),
  KEY `day` (`day`),
  KEY `profit_year` (`profit_year`),
  KEY `profit_month` (`profit_month`)
) ENGINE=InnoDB AUTO_INCREMENT=29 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='公司财务支付代理分润记录表';
...
CREATE TABLE `deliver_goods` (
  ...
  KEY `order_id` (`order_id`)
) ENGINE=InnoDB AUTO_INCREMENT=5280 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='发货表';
...
CREATE TABLE `encourage` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=14 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='励志语表';
...
) ENGINE=InnoDB AUTO_INCREMENT=245001 DEFAULT CHARSET=utf8 CHECKSUM=1 DELAY_KEY_WRITE=1 ROW_FORMAT=DYNAMIC COMMENT='标签号码表';
